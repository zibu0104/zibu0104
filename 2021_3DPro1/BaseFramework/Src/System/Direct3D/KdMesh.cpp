...
void KdMesh::SetToDevice() const
{
	...
	UINT offset = 0;					// オフセット
	D3D.WorkDevContext()->IASetVertexBuffers(0, 1, m_vertBuf.GetAddress(), &stride, &offset);

	// インデックスバッファセット
	D3D.WorkDevContext()->IASetIndexBuffer(m_indxBuf.GetBuffer(), DXGI_FORMAT_R32_UINT, 0);

	//プリミティブ・トポロジーをセット
	D3D.WorkDevContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}
...
void KdMesh::DrawSubset(int subsetNo) const
{
	...
	// 描画
	D3D.WorkDevContext()->DrawIndexed(m_subsets[subsetNo].FaceCount * 3, m_subsets[subsetNo].FaceStart * 3, 0);
}
