...
//		bufferSize = ((bufferSize + 16 - 1) / 16) * 16;
	...
	// バッファ作成
	//--------------------------------
	if (FAILED(D3D.WorkDev()->CreateBuffer(&desc, initData, &m_pBuffer)))
	{
...
void KdBuffer::WriteData(const void* pSrcData, UINT size)
{
	...
	if (m_bufUsage == D3D11_USAGE_DYNAMIC)
	{
		...
		// 書き込み専用
		if (SUCCEEDED(D3D.WorkDevContext()->Map(m_pBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
		{
			memcpy_s(pData.pData, m_bufSize, pSrcData, size);

			D3D.WorkDevContext()->Unmap(m_pBuffer, 0);
		}
	}
	// 静的バッファの場合
	...
	{
		// バッファに書き込み
		D3D.WorkDevContext()->UpdateSubresource(m_pBuffer, 0, 0, pSrcData, 0, 0);
	}
	// ステージングバッファの場合
	else if (m_bufUsage == D3D11_USAGE_STAGING)
	{
		// 読み書き両方
		D3D11_MAPPED_SUBRESOURCE pData;
		if (SUCCEEDED(D3D.WorkDevContext()->Map(m_pBuffer, 0, D3D11_MAP_READ_WRITE, 0, &pData)))
		{
			memcpy_s(pData.pData, m_bufSize, pSrcData, size);
...
void KdBuffer::CopyFrom(const KdBuffer& srcBuffer)
{
	...
	D3D.WorkDevContext()->CopyResource(m_pBuffer, srcBuffer.GetBuffer());
}
